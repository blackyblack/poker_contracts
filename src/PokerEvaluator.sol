...
library PokerEvaluator {
    ...
    function evaluateHand(
        ...
        for (uint256 i = 0; i < 7; i++) {
            ...
            rankCounts =
                (rankCounts & ~(0x0F << rankOffset)) |
                ((currentRankCount + 1) << rankOffset);
            ...
            suitCounts =
                (suitCounts & ~(0x0F << suitOffset)) |
                ((currentSuitCount + 1) << suitOffset);
        }
        ...
        if (isFlush && isStraight) {
            ...
            if (isStraightFlush) {
                ...
                return
                    (HAND_STRAIGHT_FLUSH << 20) | (uint256(straightHigh) << 16);
            }
        }
