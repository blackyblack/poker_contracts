...
- `startGame(channelId, deck)`: both players must call this function with matching decks (52 canonical base points representing a full ordered deck) for the game to be considered started. The deck consists of 52 encrypted elliptic curve points on the BN254 curve, with the first 9 cards used for the 9 slots (2 hole cards per player + 5 board cards). The remaining 43 cards are available for card-ID resolution during reveal. Both players' deck hashes must match for the game to start. Emits `GameStarted` event when both players have submitted matching hashes.
